...
class Mesh{
    ...
    init?(device : MTLDevice, Mesh : MDLMesh, vertexDescriptor : MDLVertexDescriptor , with label : String = "none"){
        self.device = device
        ...
        let VD = MTKMetalVertexDescriptorFromModelIO(vertexDescriptor)!
        
        geometryDescriptor = MTLAccelerationStructureTriangleGeometryDescriptor()
        geometryDescriptor?.vertexBuffer = vertexBuffer
        geometryDescriptor?.indexBuffer = indexBufferArray.first
        geometryDescriptor?.indexType = self.Mesh!.submeshes[0].indexType
        geometryDescriptor?.vertexStride = VD.layouts[0].stride
        geometryDescriptor?.triangleCount = self.Mesh!.submeshes[0].indexCount / 3
        
        geometryDescriptor?.primitiveDataBuffer = vertexBuffer
...
class renderPipelineState {
    ...
    init?(library : MTLLibrary , device : MTLDevice, vertexFunction : String, fragmentFunction : String, colourPixelFormat : MTLPixelFormat){
    ...
    func getState() -> MTLRenderPipelineState {
